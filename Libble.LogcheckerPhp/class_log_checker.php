...
class LOG_CHECKER {
	...
	function parse() {
		...
		$Checksum = false;
		if (preg_match("/[\=]+\s+Log checksum/i", $Log)) { //checksum
			$Checksum = true;
		...
		foreach ($this->Logs as $LogArrayKey => $Log) {
			...
			$Log = _preg_replace("/Additional command line options([^\n]{70,110} )/", "Additional command line options$1<br>", $Log);
			...
			$Log = _preg_replace(
				...
				'<span class="good">No errors occured</span>',
				$Log);
	...
	function drive($Matches){
		...
		$DriveName = $Matches[2];
		$DriveName = _preg_replace('/\s+-\s/', ' ', $DriveName);
		$DriveName = _preg_replace('/\s+/', ' ', $DriveName);
		$DriveName = _preg_replace('/[^ ]+:.*$/', '', $DriveName);
		...
		while(list($Key, $Offset) = each($Offsets)){
			$StrippedOffset = _preg_replace('/[^0-9]/s', '', $Offset);
	...
	function delete_silent_blocks($Matches){
		...
		return '<span class="log5">Delete leading and trailing silent blocks silence'.$Matches[1].'</span>: <span class="'.$Class.'">'.$Matches[2].'</span>';
	}
